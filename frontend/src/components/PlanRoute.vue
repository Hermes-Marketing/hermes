<template>
  <div class="plan-route-container">
    ...
    <p v-if="isLoading" class="loading-message">
      Calculating businesses in {{ city }}, {{ state }}, {{ country }}... Please wait.
    </p>

    <!-- Display message if no businesses are found -->
    <p v-if="!isLoading && coordinates && !cityCompanies.length" class="no-businesses">
      No businesses nearby.
    </p>

    <!-- MapDisplay Component -->
    <MapDisplay
      v-if="!isLoading && coordinates"
...
export default {
  ...
  setup() {
    ...
    const countryList = ref([
      "United States", "Canada", "United Kingdom", "Australia", "Germany", "France", "Italy", "Spain", "Mexico", "Japan"
    ]);
      ...
      try {
        ...
        if (response && response.data) {
          const filteredCompanies = response.data.filter(
            company => company.city.toLowerCase() === city.value.toLowerCase()
          );

          // Geocode the city center
          const cityCenterUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(city.value + ',' + state.value)}&format=json&limit=1`;
          const cityCenterResponse = await fetch(cityCenterUrl);
          const cityCenterData = await cityCenterResponse.json();

          if (cityCenterData.length > 0) {
            coordinates.value = [parseFloat(cityCenterData[0].lat), parseFloat(cityCenterData[0].lon)];
          }
